...
print(f"Currents max step : {round(vmax,2)} m/s")
...
# Repeat while boat is inside the plot or far from the goal
while (0 <= int(boat_pos2[0, -1:]) < size[0]) and (0 <= int(boat_pos2[1, -1:]) < size[1]) and (np.sqrt((boat_pos2[0, -1:]-goal_pos[0])**2 + (boat_pos2[1, -1:]-goal_pos[1])**2) > 1):
    # Append to boat_pos array the future position : last + current drift + direction to goal
    direction = np.arctan( (([goal_pos[1]] - boat_pos2[1,-1:]) / ([goal_pos[0]] - boat_pos2[0,-1:])) )
...
plt.contourf(X_current, Y_current, currents_map[0]**2 + currents_map[1]**2, levels=np.linspace(np.min(currents_map[0]**2 + currents_map[1]**2), np.max(currents_map[0]**2 + currents_map[1]**2),20))                            # Plot currents speed
